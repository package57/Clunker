...
The motivation behind Clunker is to de mystify computer programming, using the C++ language. Far beyond a Reference or Tutorial. Computer programming is a mysterious black box, it shouldn't be. What is a computer program. What is it made of. How is it made. Why do we even have them. What do we need to think of to make one. How do we know what it's doing. I believe I can clear all that up.
...
To accomplish both these tasks we'll write an Editor, and incorporate common tools, as a learning exercise. Because others have, we can. The Editor will be special.
...
![](../152b89c3/IBM026.jpeg)
   	
A stack of punch cards would be loaded into the IBM 026 card puncher and each line of code read from the coding sheet produced a 'punched card' by you typing the code off the sheet. The card would have small rectangular holes punched through it, in columns, that represented bytes - where ever there was a hole,that was a 1, no hole meant 0, zeros and ones, analog binary. can you say 'chad'?
...
Imagine desk checking this:

![](../caa95a47/Listing.jpg)
...
Now that you had a screen you could log on to Time Sharing Option (TS0) a CUI 'desktop' created at IBM in Toronto. From there you could manipulate settings and files. There were plenty of functions, always evolving and improving yet at the end of the day it was a CUI, no mouse, no copy / paste.  
...
The program Thing is 'implemented', coded as a function. It only has one function. The code that belongs to the function is enclosed in brackets '{}'. In fact, if you use any common coding tool, as soon as you type 'fun()' the brackets will pop up. I find that annoying. The word 'int' is a declaration of a 'return' value. You receive it. In this instance an integer. An integer is a number. We don't need to give it a name. The brackets '()' after the name of the function is where you would declare values to be sent to, or used in the function. They are called 'parameters'. The return, a C++ 'keyword', instruction sends a value back to whatever called the function. In this case, you. It must return an integer. In this example it returns a zero. It cannot return "ok", because "ok" is not an integer. When you 'run' a program, it executes the instructions. This is also referred to as 'calling' a function. C++ is very structured, yet sometimes terse. 
...
So how do we borrow code? We tell the compiler to borrow it. We code that with "<>" and put what we want to borrow from in between like this ‘#include’. Include is a pre compiler directive. We're telling C++ to do something, before, it compiles. In our Thing.cpp file. 
...
That command tells the system to display the return code of the last command on the terminal. 
...
```
We’re safe, we have a variable, a piece of memory that has an appropriate value. Typically implementations are in files, plain text, with a “.cpp” extension.
```
...
CLEditAR provides activity tracking
...
Typically data are organized into 'records' with a structure often called a layout, or a map. for example
```
struct InputFile
{
    std::string name[25];
...
![wxWidgets](https://wxwidgets.org/ "wx")
...
Everything is happy. Did I mention you need to install wxWidgets, MySQL and C++ connection. It’s easy, I did it. Actually, the most helpful site for solving technical issues is ChatGPT.com. Just go there and type for example 'solve install MySQL Linux' and you'll have the answers. It's 1000 times better than stackoverflow. 
...
Write your program as clearly as possible. Take as many lines as you want. Code as many lines as you want. Replicate functions. No matter. The compiler and modern cpu’s will make your worst logic efficient. The computer will always understand what you want. CPU's get faster and faster almost daily.    
...
implement in CLEditAR.cpp. nothing fancy, just read a char 

```
void CLEditAR::Read()
{

    LogFile << "Read CLEditAR.txt " << std::endl;
...
void CLEditAR::Close()
{
    ...
    LogFile << "Close CLEditAR.txt " << std::endl;
...
void CLEditAR::OpenO()
{
    ...
    LogFile << "Open CLEditAR.txt Output " << std::endl;
...
and get in CF.cpp and DB.cpp action
in CF after init()
...
let's do an open and a from stage and look at the logs to see if we can make progress on the reports.
Main

```
Welcome pierre Sun Dec 22 03:07:59 2024
Command fromstage
Parameter one mydb
...
rows 1
From Stage elapsed time: 17.63
Welcome pierre Sun Dec 22 03:13:58 2024
Command open
...
Error file size 0
Open File objects.h
Usage Date 20241220 0000000001
File In Bytes 506
File In Records 23
File Out Bytes 0
File Out Records 0
...
Count(*) 1 testones
From Stage mydb testones
Usage Date 20241220 0000000002
From Stage elapsed time: 0.269
```

something is wrong
it the act react file exists, but is empty or the wrong size, you get a Date and the seq is alawsy 1, and the file never has data!
...
WOW. Ok. That was really hard to find. Thank God we could focus on one file.
...
// close Main log file
// dump the Main log into a table, create the anchor points 
// process CF log file, connect to anchor point
// open CF log file for input
// read byte by byte
// process until end of file, aka eof   
// close CF log file
// dump the CF log into a table, attach to the anchor points
// process DB log file, connect to anchor point
// open DB log file for input
// read byte by byte
// process until end of file, aka eof   
// close DB log file
// dump the DB log into a table, attach to the anchor points
```
If you've been following so far, 90% of what we need to do exists in code in CF and or DB. This is normal, and a huge time saver.

the process the Main log file would look like this
...
the process the DB log file would look like this
...
// collect those structures into an array
```
dump the Main log array into a Table, create the anchor points 

dump the CF log array into a Table, attach to the anchor points
...
start by creating a 'dummy' class and connecting it to the template CLEditTemp.
add in CLEditTemp.h
```
...
// close log file
// like Bind
// loop,dump the log array into a Table, create, or update, the anchor points 
// like ToStage
```

in CLEditXETL.h 
declare take a copy of DB.h and CF.h and remove the duplication 

in CLEditXETL.cpp
implement take a copy of DB.cpp and CF.cpp and remove the duplication 
the 'build' function in code::blocks will help you out
 
For CLEditXETL.h there will be one public function; not DoStuff() too silly
delcare
int ETL();
...
// loop,process until end of file, aka eof 
// 	in each record, look for key words like Welcome, Usage, Elapsed etc
// 	gather those meta data into a structure
// 	take each structures and INSERT or UPDATE rows in a table
// eop
...
You'll notice I'm sure by now that data processing has a logic framework. It goes like this, initial, open, read, process until end of file, end of process, close. Here we're using file processing and database processing, so the connection to the database, Bind() follows the open(s), and the database Free() preceeds the close(s). If you follow that framework you'll never go wrong.
...
The initialize opens the files and provides an inital value, initial implementation of all your variables. Many of the boolean type are there just to wrangle the logging. Ideally, there would be a 'Logging' action in Main and we would pass that value around and trim the fat off the logs. That's a great idea, LogOn(), LogOff()! 
...
$ sudo mysql
mysql> USE mydb;
mysql> SELECT * FROM mydb.editxetl;
```

![alternate text][id]
[id]: ./Mysql1.png "Actions"

Here I've rolled the logs from CF and DB too. 

![alt text](Mysql2.png "Reactions")

Full disclosure. I decided to make some changes I thought made sense after all these days of testing. I added a flag 
```
bool Logging;
```
as public in all the classes and in Main. I added a file 
```
...
{
// speak a string
    system("espeak-ng -ven-us -g 8 \"Lily likes to be silly \" ");
// speak a file
    system("espeak-ng -ven-us -g 8 -f booboo.txt ");
    return 0;
}
```
